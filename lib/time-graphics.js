	...
	var Graphic = function(sDivId){ //constructor
		...
		var _refreshChartInfo = function(){ //update oChart to adjust all funcitons to chart. Like zoom restart
			...
			for (var i = 0; i < oChart.aFunctions.length; i++) {
				...
				if(oChart.aFunctions[i].data[0].date < since){
					since = oChart.aFunctions[i].data[0].date;
				}
				if(oChart.aFunctions[i].data.slice(-1)[0].date > until){
					until = oChart.aFunctions[i].data.slice(-1)[0].date;
				}
			}
			/*oChart.max = max;
			oChart.min = min;*/
			if (since === Infinity || !until || !since) {
				console.log('Warning. Range not correct.');
			}
			oChart.since = since;
			oChart.until = until;
		};
		...
		var _writeValues = function(nCursorX, nCursorY){ //write values at the right of the chart
		...
		var _addNewData = function(oFunction, aNewData){ //sort and map data
			...
			var fnFilterValue = function(oVal){
				if (oChart.since && oChart.until) {
					return oVal.date >= oChart.since && oVal.date <= oChart.until;
				}
				return true;
			};
			oFunction.max = _getMax(oFunction.data.filter(fnFilterValue), 'value');
			oFunction.min = _getMin(oFunction.data.filter(fnFilterValue), 'value');
		};
		...
		oGraphic.zoom = function(oRange){
			oChart.since = oRange.since; //new range
			oChart.until = oRange.until;
			var fnFilterValue = function(oVal){
				return oVal.date >= oChart.since && oVal.date <= oChart.until;
			};
			for (var i = 0; i < oChart.aFunctions.length; i++) { //update the max and min of each function
				oChart.aFunctions[i].min = _getMin(oChart.aFunctions[i].data.filter(fnFilterValue), 'value');
				oChart.aFunctions[i].max = _getMax(oChart.aFunctions[i].data.filter(fnFilterValue), 'value');
				if (oChart.aFunctions[i].max <= oChart.aFunctions[i].min) {
					console.error('Error while zooming the chart.');
					oChart.aFunctions[i].min = -1;
					oChart.aFunctions[i].max = 1;
				}
			}
			_drawCharts();
		};
